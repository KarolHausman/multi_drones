...
class Rotor2dMotionModel : public MultiAgentMotionModel {
public:
  Rotor2dMotionModel();
  virtual ~Rotor2dMotionModel();
  virtual void init(const TParam &p);

  //! returns the resulting state [x_1, y_1, theta_1, ..., x_N, y_N, theta_N, x_N+1, y_N+1, v_x_N+1, v_y_N+1] (in global system)
  //! given the applied control [u_x_1, u_y_1, u_v_theta_1, ..., u_x_N, u_y_N, u_v_theta_N] (in global system)
  //! and the noise [n_v_x_1, n_v_y_1, n_v_theta_1..., n_v_x_N, n_v_y_N, n_v_theta_N, n_a_x_N+1, n_a_y_N+1] (in global system)
